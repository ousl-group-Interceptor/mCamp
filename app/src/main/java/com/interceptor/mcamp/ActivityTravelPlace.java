...
public class ActivityTravelPlace extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_traval_place);
    ...
    private void manageLocation(boolean[] run) {
        ...
        Data.child("Locations/Place").addValueEventListener(new ValueEventListener() {
            @Override
        ...
        Data.child("Locations/Place").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (run[0]) {
                    ...
                    Common.stopLoading();
                    if (data.size() == 0)
                        Toast.makeText(ActivityTravelPlace.this, "Nothing was found", Toast.LENGTH_LONG).show();
                    for (int i = data.size() - 1; i >= 0; i--) {
                        DataSnapshot snapshot = data.get(i);
                        locationAdder(snapshot.getKey(),
                                String.valueOf(snapshot.child("Date").getValue()),
