...
public class ActivityTransportation extends AppCompatActivity {
    ...
    private void manageLocation(boolean[] run) {
        ...
        Data.child("Locations/Transportation").addValueEventListener(new ValueEventListener() {
            @Override
        ...
        Data.child("Locations/Transportation").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (run[0]) {
                    ...
                    Common.stopLoading();
                    if (data.size() == 0)
                        Toast.makeText(ActivityTransportation.this, "Nothing was found", Toast.LENGTH_LONG).show();
                    for (int i = data.size() - 1; i >= 0; i--) {
                        DataSnapshot snapshot = data.get(i);
                        locationAdder(snapshot.getKey(),
                                String.valueOf(snapshot.child("name").getValue()),
